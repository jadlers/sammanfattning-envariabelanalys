...
Om $F(x)$ är en primitiv till $f(x)$ och $g(x)$ är deriverbar. Då kommer $F(g(x))$ vara en primitiv funktion till $f(g(x))g'(x)$. Variabelsubstitution används för att beräkna integraler på form $\int_{a}^{b}f(g(x))g'(x)dx=F(g(x))\bigg\vert_{a}^{b}$
...
\subsubsection{Förklaring}
Partiell integration följer av derivatans produktregel. Enligt produktregeln så ska $D(f(x)g(x))=f'(x)g(x)+f(x)g'(x)$. Hittar vi sedan den primitiva funktionen till $f(x)g(x)=\int f'(x)g(x)dx + \int f(x)g'(x)dx$ som kan skrivas om till:\\
$\int_{a}^{b}f(x)g(x)dx=f(x)G(x)\bigg\vert_{a}^{b} - \int_{a}^{b}f'(x)G(x)dx$
